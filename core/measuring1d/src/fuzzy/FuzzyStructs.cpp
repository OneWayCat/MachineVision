...
// Constructor
FuzzyFunction::FuzzyFunction(const std::vector<double> &xVals_, const std::vector<double> &yVals_) {
    // Validate size of vectors
    size_t n = xVals_.size();

    if (n != yVals_.size() || n < 2)
...
// Interpolation
double FuzzyFunction::interpolate(double x) const {
    if (x <= xVals.front())
        return yVals.front();
...
void FuzzyFunction::scale(double factor) {
    if (factor <= 0)
