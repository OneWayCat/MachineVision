...
PosDistanceResult::PosDistanceResult(std::vector<cv::Point2d> &pos_, std::vector<double> &distances_) :
        pos{std::move(pos_)},
        distances{std::move(distances_)} {
}

PosDistanceResult::PosDistanceResult(PosDistanceResult &&obj) noexcept:
        pos{std::move(obj.pos)},
        distances{std::move(obj.distances)} {
}

MeasurePosResult::MeasurePosResult(PosDistanceResult &pos, std::vector<double> &amplitudes_) :

        PosDistanceResult{std::move(pos)},
        amplitudes{std::move(amplitudes_)} {
}

MeasurePairsResult::MeasurePairsResult(std::vector<cv::Point2d> &posFirst_, std::vector<double> &amplitudesFirst_,
...
MeasurePairsResult MeasurePairsResult::front() {
    ...
    return MeasurePairsResult{pf, af, ps, as, intra, inter};
}

MeasurePairsResult MeasurePairsResult::back() {
...
FuzzyMeasurePosResult::FuzzyMeasurePosResult(PosDistanceResult &pos, std::vector<double> &amplitudes_,
                                             std::vector<double> &scores_) :

        MeasurePosResult(pos, amplitudes_),
...
FuzzyMeasurePairsResult::FuzzyMeasurePairsResult(std::vector<cv::Point2d> &posFirst, std::vector<double> &ampFirst,
        ...
        MeasurePairsResult{posFirst, ampFirst, posSecond, ampSecond, intraDist, interDist},
